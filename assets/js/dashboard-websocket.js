...
var sendAnswer = function (msgObj) {
	socket.emit("questionSent", msgObj);
};
var sendQuestionStatus = function (status) {
	socket.emit("questionStatus", status);
};
var sendQuestionDelete = function (deleteQuestion) {
	socket.emit("questionDelete", deleteQuestion);
};
...
var sendEventStatus = function (eventStatus) {
	socket.emit("eventStatus", eventStatus);
};

var sendContestStatus = function (contestStatus) {
	socket.emit("contestStatus", contestStatus);
};
var sendContestQuestionStatus = function (contestQuestionStatus) {
	socket.emit("contestQuestionStatus", contestQuestionStatus);
};
...
var sendPollStatus = function (pollStatus) {
	socket.emit("pollStatus", pollStatus);
};
var sendPollVisibility = function (pollVisibility) {
	socket.emit("pollVisibility", pollVisibility);
};
...
moduleSwitches.forEach(function (moduleSwitch) {
	moduleSwitch.addEventListener("change", function () {
		...
		console.log("changeStatus : ", moduleSwitch.dataset.switch);
	});
});
...
var receiveQuestion = function (obj) {
	...
	if (questionReceiverBackoffice) {
		if (objReceived.status == true) {
			var switchStatus = "checked";
		}
		var createdReceiver = document.createElement("div");
		createdReceiver.id = "question-card-" + objReceived.id;
		createdReceiver.className = "col-md-6 mb-4 question-card";
		createdReceiver.innerHTML += `
            <div class="card" id="card-${objReceived.id}">
                <div class="card-header d-flex">
                    <span class="me-auto">
						${objReceived.user.firstname} ${objReceived.user.lastname}<br>
						${objReceived.user.email}
					</span>
                    
                    <a class="js-status">
                        <span class="js-question-status">
                            <div class="form-check form-switch">
                                <input class="form-check-input js-question-switch" type="checkbox" data-switch="${objReceived.id}" id="switch-${objReceived.id}" ${switchStatus}>
                            </div>
                        </span>
                    </a>
                    
                    <a class="reply ms-4" href="/answer/question/${objReceived.id}" data-bs-toggle="modal" data-bs-target="#answerModal-${objReceived.id}">
                        <i class="fal fa-reply text-warning"></i>
                    </a>
                    <a class="js-delete ms-4" data-question="${objReceived.id}" id="delete-btn-${objReceived.id}">
                        <i class="fal fa-trash-alt text-danger"></i>
                    </a>
                </div>
                <div class="card-body">
                    <p class="card-text">${objReceived.question}</p>
                </div>
            </div>
            <!-- Modal -->
            <div class="modal fade" id="answerModal-${objReceived.id}" tabindex="-1" aria-labelledby="answerModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="answerModalLabel">${objReceived.question}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="js-answer-${objReceived.id}" class="form-label">Votre réponse</label>
                                <textarea class="form-control" id="js-answer-${objReceived.id}" rows="3"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                            <button type="button" class="btn btn-primary answer-btn" id="js-answer-button-${objReceived.id} " data-question="${objReceived.id}" data-bs-dismiss="modal">Envoyer</button>
                        </div>
                    </div>
                </div>
            </div>
